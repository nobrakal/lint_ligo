...
let print_braced loc f {lbrace; inside; rbrace} =
  node Braces (K.lbrace lbrace :: f inside @ [K.rbrace rbrace]) loc
...
let print_terminator x =
  opt_to_list (fun x -> [K.semi x]) x
...
and print_pcons x =
  let {lbracket; lpattern; comma; ellipsis; rpattern; rbracket} = x.value in
  [K.lbracket lbracket; print_pattern x.region lpattern; K.comma comma;
  K.ellipsis ellipsis; print_pattern x.region rpattern; K.rbracket rbracket]
...
and print_arguments = function
  | Multiple xs ->
     [print_par (fun x -> print_nsepseq K.comma (print_expr xs.region) x) xs]
  | Unit     x  ->
     print_unit x.value
...
and print_call e =
  ...
  print_expr loc lambda::print_arguments args
...
and print_update x =
  let {lbrace; ellipsis; record; comma; updates; rbrace} = x.value in
  let updates = print_ne_injection (print_field_path_assign) updates.value in
  let record  = print_path x.region record in
  [K.lbrace lbrace; K.ellipsis ellipsis; record; K.comma comma]
  @ updates @ [K.rbrace rbrace]
...
| Reasonligo.CST.Braces (l,r)   -> (K.lbrace l,K.rbrace r)
...
and print_let_in x =
  let {kwd_let; binding; kwd_rec; body; semi; attributes} = x.value in
  let value = (kwd_let, kwd_rec, binding, attributes) and region = x.region in
  print_let_decl {value;region} ::
    [K.semi semi; print_expr x.region body]

and print_fun x =
  let {binders; lhs_type; arrow; body} = x.value in
  print_pattern x.region binders
...
and print_list = function
  | ECons x ->
     let {lbracket; lexpr; comma; ellipsis; rexpr; rbracket} = x.value in
     [K.lbracket lbracket; print_expr x.region lexpr; K.comma comma;
      K.ellipsis ellipsis; print_expr x.region rexpr; K.rbracket rbracket]
...
and print_comp_expr = function
  ...
  | Equal e -> print_bin_op K.equal_comp e
...
and print_cond x =
  let {kwd_if; test; ifso; ifnot} = x.value in
  [K.kwd_if kwd_if; print_expr x.region test]
  @ [print_braced x.region
       (fun (expr,semi) ->
         print_expr x.region expr
         :: print_terminator semi) ifso]
  @ opt_to_list
      (fun (kwd_else, block) ->
        [ K.kwd_else kwd_else;
          print_braced x.region
           (fun (expr,semi) ->
             print_expr x.region expr
             :: print_terminator semi) block]) ifnot

and print_case_expr x =
  let {kwd_switch; expr; lbrace; cases; rbrace} = x.value in
  [K.kwd_switch kwd_switch; print_expr x.region expr; K.lbrace lbrace]
  @ print_cases cases
  @ [K.rbrace rbrace]

and print_cases xs = print_nsepseq K.vbar print_clause xs.value

and print_clause x =
  let {pattern; arrow; rhs; terminator} = x.value in
  let xs =
    [print_pattern x.region pattern; K.arrow arrow; print_expr x.region rhs]
      @ print_terminator terminator in
  node Clause xs x.region
...
let declaration = function
  | ConstDecl xs -> print_let_decl xs
  | TypeDecl xs -> print_type_decl xs

let unparse_cst : Reasonligo.CST.t -> node Ast.t list =
